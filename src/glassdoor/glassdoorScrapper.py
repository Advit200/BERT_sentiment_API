...
DEFAULT_URL = (
    'https://www.glassdoor.co.in/Overview/Working-at-Koch-Industries-EI_IE2864.11,26.htm')
...
parser.add_argument('-l', '--limit', default=2500,
                    action='store', type=int, help='Max reviews to scrape, Min is 10')
...
if not args.start_from_url and (args.max_date or args.min_date):
    args.start_from_url = 'https://www.glassdoor.co.in/Reviews/Koch-Industries-Reviews-E2864.htm?sort.sortType=RD&sort.ascending=true&filter.iso3Language=eng'
...
if args.credentials:
    ...
    try:
        with open('./glassdoor/secret.json') as f:
            d = json.loads(f.read())
            args.username = d['username']
            args.password = d['password']
...
def scrape(field, review, author):
    ...
    def scrape_date(review):
        date = author.find_element_by_class_name(
    ...
    def scrape_city(review):
        if 'in' in review.text:
            try:
                res = author.find_element_by_class_name(
                    'authorLocation').text.split(',')[0]
            except Exception:
                logger.warning('Failed to scrape employee_city')
                res = np.nan
        else:
            res = "N/A"
        return res

    def scrape_country(review):
        if 'in' in review.text:
            try:
                res = author.find_element_by_class_name(
                    'authorLocation').text.split(',')
                if len(res) > 1:
                    res = res[1]
                elif ('bangalore' in res[0].lower() or 'bengaluru' in res[0].lower() or 'mumbai' in res[0].lower() or 'vadod' in res[0].lower() or 'delhi' in res[0].lower()):
                    res = 'India'
                else:
                    res = ''
            except Exception:
                logger.warning('Failed to scrape employee_country')
                res = np.nan
        else:
            res = "N/A"
        return res
    ...
    def scrape_status(review):
        try:
            res = review.find_element_by_class_name('pt-xsm').text
        except Exception:
    ...
    def scrape_rev_title(review):
        title = review.find_element_by_class_name('mb-xxsm').text
        title = title.replace("-", " ")
        return title
    ...
    def scrape_pros(review):
        try:
            ...
            # if pros.text.find('Pros'):
            #     res = review.find_element_by_xpath(
            #         '//*/div/div/div[2]/div/div[2]/div[1]/p[2]/span').text
            # else:
            #     res = ''
    ...
    def scrape_cons(review):
        try:
            ...
            # if cons.text.find('Cons'):
            #     res = review.find_element_by_xpath(
            #         '//*/div/div/div[2]/div/div[2]/div[2]/p[2]/span').text
            # else:
            #     res = ''
            continue_index = cons.text.find('Advice to Management')
            if continue_index == -1:
                continue_index = cons.text.find(
                    'people found this review helpful')-2
            res = cons.text[con_index+5: continue_index]
            if "people found this review helpful" in res:
                res = res.split("people found this review helpful")[0]
                res = res[:-2]
            if "Be the first to find this review helpful" in res:
                res = res.split(
                    "Be the first to find this review helpful")[0]
        except Exception:
            res = np.nan
        return res
    ...
    def scrape_advice(review):
        try:
            ...
            location = advice.text.find('person found this review helpful')-2
            if location == -1:
                location = advice.text.find(
                    'Be the first to find this review helpful')
            # if advice.text.find('Advice to Management'):
            #     res = review.find_element_by_xpath(
            #         '//*/div/div/div[2]/div/div[2]/div[3]/p[2]/span').text
            # else:
            #     res = ''
            if advice_index != -1:
                res = advice.text[advice_index+21: location]
                if "people found this review helpful" in res:
                    res = res.split("people found this review helpful")[0]
                    res = res[:-2]
                if "Be the first to find this review helpful" in res:
    ...
    def scrape_recommends(review):
        try:
            innerHTMLObj = review.get_attribute('innerHTML')
            innerHTMLObj = innerHTMLObj.split(
                '<div class="d-flex my-std reviewBodyCell recommends">')[1]
            innerHTMLObj = innerHTMLObj.split('Recommend</span>')[0]
            res = ''
            if "1kiw93k" in innerHTMLObj:
                res = "No"
            if "hcqxoa" in innerHTMLObj:
                res = "Yes"
            if "1h93d4v" in innerHTMLObj:
                res = "May Be"
            if "10xv9lv" in innerHTMLObj:
                res = "Dont Know"
            return res
        except:
            return np.nan
    ...
    def scrape_outlook(review):
        try:
            ...
            #     'd-flex my-std reviewBodyCell recommends').text
            # res = res.split('\n')
            ...
            innerHTMLObj = innerHTMLObj.split(
                '<div class="d-flex my-std reviewBodyCell recommends">')[1]
            innerHTMLObj = innerHTMLObj.split('CEO Approval</span>')[1]
            innerHTMLObj = innerHTMLObj.split('Business Outlook</span>')[0]
            res = ''
            if "1kiw93k" in innerHTMLObj:
                res = "No"
            if "hcqxoa" in innerHTMLObj:
    ...
    def scrape_approve_ceo(review):
        try:
            ...
            innerHTMLObj = innerHTMLObj.split(
                '<div class="d-flex my-std reviewBodyCell recommends">')[1]
            innerHTMLObj = innerHTMLObj.split('Recommend</span>')[1]
            innerHTMLObj = innerHTMLObj.split('CEO Approval</span>')[0]
            res = ''
            if "1kiw93k" in innerHTMLObj:
                res = "No"
            if "hcqxoa" in innerHTMLObj:
    ...
    def scrape_employer_responseBy(review):
        try:
            innerHTMLObj = review.get_attribute('innerHTML')
            innerHTMLObj = innerHTMLObj.split(
                '<div class="commentHeader px-std mb-xsm"><p class="my-0"><span class="strong">')[1]
            return innerHTMLObj.split('Response</span></p><p')[0]
        except:
            return np.nan
    def scrape_employer_responder(review):
        try:
            innerHTMLObj = review.get_attribute('innerHTML')
            innerHTMLObj= innerHTMLObj.split('Response</span></p><p')[1]
            innerHTMLObj= innerHTMLObj.split('>')[1]
            innerHTMLObj = innerHTMLObj.split('</p>')[0]
            innerHTMLObj = innerHTMLObj.replace('amp;','')
            innerHTMLObj = innerHTMLObj.replace('</p','')
            return innerHTMLObj
        except:
            return np.nan
    def scrape_employer_response(review):
        try:
            innerHTMLObj = review.get_attribute('innerHTML')
            innerHTMLObj= innerHTMLObj.split('<span data-test="">')[1]
            innerHTMLObj= innerHTMLObj.split('</span>')[0]
            return innerHTMLObj
        except:
            return np.nan
    ...
    funcs = [
        ...
        scrape_outlook,
        scrape_approve_ceo,
        scrape_employer_responseBy,
        scrape_employer_responder,
        scrape_employer_response
    ]
...
def sign_in():
    logger.info(f'Signing in to Glassdoor usnig {args.username}')
...
def get_browser():
    ...
    browser = wd.Chrome(options=chrome_options,
                        executable_path=r'.\glassdoor\chromedriver.exe')
    return browser
...
def verify_date_sorting(data):
    logger.info('Date limit specified, sorting the reviews')
    sortedURL = data + "?sort.sortType=RD&sort.ascending=false&filter.iso3Language=eng"
    browser.get(sortedURL)
...
def main():
    ...
    sign_in()
    if not args.max_date or not args.min_date:
        ...
        if not reviews_exist:
            return
        verify_date_sorting(reviews_exist)
        # browser.get(args.url)
